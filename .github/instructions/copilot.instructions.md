...
| Repo | Purpose |
|------|---------|
| [`hexadian-auth-service`](https://github.com/Hexadian-Corporation/hexadian-auth-service) | Centralized identity platform — issues the JWTs this library validates. Token architecture, RBAC, auth code flow. || [`hexadian-auth-client`](https://github.com/Hexadian-Corporation/hexadian-auth-client) | TypeScript auth client SDK (npm monorepo). Four packages including `@hexadian-corporation/auth-node` — the TypeScript server-side equivalent of this library (JWT verification, permission checking, Express/NestJS middleware). Also: `auth-core` (OAuth client, token storage), `auth-react` (React integration), `auth-angular` (Angular 17+ integration). Published to GitHub Packages. |
## Package Structure
